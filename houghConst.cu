...
__constant__ float d_Cos[degreeBins]; // Declaración en memoria constante
__constant__ float d_Sin[degreeBins]; // Declaración en memoria constante
...
__global__ void GPU_HoughTranConst(unsigned char *pic, int w, int h, int *acc, float rMax, float rScale)
{
  ...
  if (pic[gloID] > 0)
  {
    ...
    {
      // calculo con memoria constante para calcular el radio
...
int main(int argc, char **argv)
{
  ...
  // color con el que vamos a colorear la imagen
  ...
  // llamada al kernel
  GPU_HoughTranConst<<<blockNum, blockDim>>>(d_in, w, h, d_hough, rMax, rScale);
  ...
  // guardar imagen
  inImg->saveImg("Constante.jpeg", lines, radInc, rBins);
